...
import {PATTERN_SORTS, PATTERN_TYPES, Root, Word} from "ogorasso";
    ...
    <div className="word-row" id={(first) ? root[0] : undefined}>
      ...
      <div className="word-row-main" {...data({borrowed})}>
        ...
        {PATTERN_SORTS.map((patternSort) => PATTERN_TYPES.map((patternType) => (
          <WordCell key={patternSort + "-" + patternType} root={root} patternSort={patternSort} patternType={patternType} words={words}/>
        )))}
      </div>
    </div>
  );
